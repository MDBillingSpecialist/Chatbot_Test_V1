...
def segment_pdf_heuristically(pdf_path: str) -> Dict[str, Dict]:
    """Extract and segment the text from the PDF using heuristic methods."""
    logging.info("Starting heuristic PDF segmentation process.")
    text_segments = {}
    current_title = None
    current_segment = []
    start_page = None
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            with tqdm(total=total_pages, desc="Processing PDF", unit="page") as pbar:
                for page_num in range(total_pages):
                    ...
                    for line in lines:
                        # Heuristic: Identify headings based on common patterns (e.g., all caps, bold, larger font size)
                        if re.match(r'^[A-Z\s]+$', line.strip()):
                            ...
                            current_title = f"Segment {segment_counter}: {line.strip()}"
                            segment_counter += 1
                            start_page = page_num + 1
                            logging.debug(f"Heading detected: '{current_title}' - Creating new segment.")
                        else:
                            if current_title:
                                current_segment.append(line.strip())
        ...
        logging.error(f"Error during heuristic PDF processing: {e}")
        return {}

    logging.info("Heuristic PDF segmentation completed successfully.")
    return text_segments
...
def process_all_pdfs_in_directory(raw_data_dir: str, output_dir: str, toc_json_path: str = None):
    """Process all PDF files in the raw data directory."""
    for filename in os.listdir(raw_data_dir):
        if filename.endswith(".pdf"):
            pdf_path = os.path.join(raw_data_dir, filename)
            output_file = os.path.join(output_dir, f"{os.path.splitext(filename)[0]}_segmented.json")
            ...
            if segments:
                ...
                logging.info(f"Segmentation completed for {filename}. Output saved to: {output_file}")
            else:
                logging.error(f"Error: No segments were extracted for {filename}. Please check the log file for details.")
...
def main():
    ...
    process_all_pdfs_in_directory(raw_data_dir, output_dir, toc_json_path)
