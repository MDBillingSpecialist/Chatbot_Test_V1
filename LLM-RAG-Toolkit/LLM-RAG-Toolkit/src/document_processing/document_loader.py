    ...
    def load_document(self, file_path: str) -> Document:
        ...
        try:
            ...
            document = Document(
                text=content,
                doc_id=file_path,
                extra_info={"file_type": file_extension}
            )
            # Remove quality_check as LlamaIndex handles this internally
            return document
        except Exception as e:
            logger.error(f"Error loading document {file_path}: {e}", exc_info=True)
            return Document(text="", doc_id=file_path, extra_info={"error": str(e)})
